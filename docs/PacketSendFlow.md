...
{
    session->Send(sendBuffer);  // 특정 세션에게만 전송
}
```
...
- **같은 SendBuffer를 여러 세션에 재사용** 가능 (메모리 효율)
- `weak_ptr::lock()`으로 세션이 유효한지 확인 후 전송
- `exceptId`로 특정 플레이어 제외 가능 (자기 이동은 자기에게 안 보냄)

---

## 2단계: SendBuffer 생성

**파일:** `GameServer/ServerPacketHandler.h:85-102`
...
### 패킷별 MakeSendBuffer 오버로드

```cpp
// ServerPacketHandler.h:63-71
...
이렇게 하면 사용할 때 패킷 ID를 직접 지정하지 않아도 됩니다:
```cpp
// 편리하게 사용
auto buffer = ServerPacketHandler::MakeSendBuffer(movePkt);  // ID 자동 설정
```
...
**파일:** `ServerCore/Session.cpp:27-61`
...
**파일:** `ServerCore/Session.cpp:180-222`
...
**파일:** `ServerCore/Session.cpp:276-295`
...
| 단계 | 파일 | 라인 | 함수명 |
|------|------|------|--------|
| 1 | `GameServer/Room.cpp` | 263-284 | `Room::HandleMove()` 등 |
| 1 | `GameServer/Room.cpp` | 350-364 | `Room::Broadcast()` |
| 2 | `GameServer/ServerPacketHandler.h` | 85-102 | `MakeSendBuffer<T>()` |
| 3 | `ServerCore/Session.cpp` | 27-61 | `Session::Send()` |
| 4 | `ServerCore/Session.cpp` | 180-222 | `Session::RegisterSend()` |
| 5 | `ServerCore/Session.cpp` | 276-295 | `Session::ProcessSend()` |
...
암호화가 활성화된 경우 `Send()`에서 `EncryptBuffer()`가 호출됩니다.

```cpp
SendBufferRef Session::EncryptBuffer(SendBufferRef sendBuffer)
{
    // 원본: [size(2)][id(2)][flags(1)][seq(4)][data...]
    // 결과: [size(2)][encrypted(id+flags+seq+data)][HMAC(32)]

    int32 plainSize = sendBuffer->WriteSize();
    int32 payloadSize = plainSize - sizeof(uint16);  // size 제외

    // 암호화된 페이로드 크기 계산 (AES 블록 패딩)
    ...
    *(reinterpret_cast<uint16*>(bufferPtr)) = static_cast<uint16>(totalSize);
    ...
    // id+flags+seq+data 부분 암호화
    _crypto->Encrypt(
        sendBuffer->Buffer() + sizeof(uint16),
        payloadSize,
    ...
    _crypto->ComputeHMAC(
        bufferPtr + sizeof(uint16),
        encryptedPayloadSize,
        bufferPtr + sizeof(uint16) + encryptedPayloadSize
    );
